...
```{r, fig.width=8, fig.height=8}
ScatterPlot(frm, "x", "y", title="Example Fit")
```
...
```{r, fig.width=8, fig.height=8}
ScatterPlot(frm, "x", "y", smoothmethod="identity", title="Example Relation Plot", annot_size=2)
```

```{r, fig.width=8, fig.height=8}
BinaryYScatterPlot(frm, "x", "posY", title="Example Binary Y Plot")
```
...
ClevelandDotPlot(randomDraws, "letter", limitN = 10,  title = "Top 10 most frequent letters")
